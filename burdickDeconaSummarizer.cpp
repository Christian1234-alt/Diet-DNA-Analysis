				...
				)
{
  fprintf
    (outPtr,
     ...
     FLAG_PREFIXES[0],SUMMARIZE_OUTPUT_NAME_ARRAY[1],
     ( (DEFAULT_SUMMARIZE_OUTPUT == SummarizeOutput::CSV)
       ? "(this is the default)"
       : ""
     ),
     OUTPUT_FILENAME_STR,SUMMARIZE_OUTPUT_NAME_ARRAY[0],
     OUTPUT_FILENAME_STR,SUMMARIZE_OUTPUT_NAME_ARRAY[1]
    );
}


void		handleSamples	(ByteBuffer&	buffer,
				 FILE*		demuxFilePtr,
				 DIR*		resultsDirPtr,
				 const char*	resultsDirCPtr
				)
{
  ...
  while  (fgets(line,MAX_LINE,demuxFilePtr) != nullptr)
  {
    char	sampleName[MAX_LINE];
    const char*	sourceCPtr	= line;
    char*	destCPtr	= sampleName;
    bool  	isDoubleQuoted	= false;

    //  Note quoted names:
    if  (*sourceCPtr == QUOTE_CHAR)
    {
      isDoubleQuoted	= true;
      sourceCPtr++;
    }

    //  Only copy names that look like identifiers:
    while  ( (sourceCPtr - line) < (MAX_LINE-1) )
    {
      if  (!isalnum(*sourceCPtr) && (*sourceCPtr!='_') && (*sourceCPtr!='-'))
        break;
    ...
    //  Disallow mal-formed names:
    if  ( ( isDoubleQuoted && (*sourceCPtr != QUOTE_CHAR))	||
	  ( (sourceCPtr - line) >= (MAX_LINE-1) )		||
	  (*sourceCPtr != ',')
	)
    {
      continue;
    ...
    //  Assume name is good:
    *destCPtr	= '\0';
    handleOneSample(buffer,sampleName,resultsDirPtr,resultsDirCPtr);
  }
}


void		obtainGlobalVarValues
				(int		argc,
				 char*		argv[]
				)
{
  //  II.  Attempt to set member vars:
  //  II.A.  Attempt to set 'minFractionalIdMatch_global':
  char*	cPtr;

  if  (argc >= 4)
  {
    minFractionalIdMatch_global	= strtod(argv[3],&cPtr);

    if  ( (minFractionalIdMatch_global < 0.0)    ||
    	  (minFractionalIdMatch_global > 100.0)  ||
	  (*cPtr		       != '\0')
	)
    {
      fprintf(stderr,"Illegal threshold value.\n\n");
      showHelp(stderr);
      exit(EXIT_FAILURE);
    }

    if  (minFractionalIdMatch_global > 1.0)
    {
      minFractionalIdMatch_global	/= 100.0;
    }
  }

  //  II.B.  Attempt to set 'summarizeOutput_global':
  if  (argc >= 5)
  {
    int		index;
    const char*	prefixCPtr;

    for  (index = 0;  index < (int)NUM_FLAG_PREFIXES;  index++)
    {
      prefixCPtr	= FLAG_PREFIXES[index];

      if  (strncmp(prefixCPtr,argv[4],strlen(prefixCPtr)) == 0)
      {
	break;
      }
    }

    if  (index >= NUM_FLAG_PREFIXES)
    {
    ...
    const char*	flagCPtr	= argv[4] + strlen(prefixCPtr);

    for  (index = 0;  index < NUM_SUMMARIZE_OUTPUT_NAME_ARRAY;  index++)
    {
      if  (strcasecmp(SUMMARIZE_OUTPUT_NAME_ARRAY[index],flagCPtr) == 0)
      {
	break;
      }
    }

    if  (index >= NUM_SUMMARIZE_OUTPUT_NAME_ARRAY)
    {
    ...
    summarizeOutput_global	= static_cast<SummarizeOutput>(index);
  }
				...
				)
{
  ...
  ByteBuffer	buffer;
  const char*	demuxFilePathCPtr	= argv[1];
  char*		resultsDirCPtr		= argv[2];

  obtainGlobalVarValues(argc,argv);

  FILE*		demuxFilePtr		= openDemuxFile(demuxFilePathCPtr);
  DIR*		resultDirPtr		= openResultsDir(resultsDirCPtr);
  int		outFileFd		= openOutputFd(resultsDirCPtr);

  if  ( doesEndWith(resultsDirCPtr,"/") )
  {
    resultsDirCPtr[strlen(resultsDirCPtr)-1]	= '\0';
  }

  csvFirstColumnsCArray_global[0]		= '\0';
  ...
  case SummarizeOutput::CSV :
    buffer.append
		("\"sample\",\"cluster\",\"numHits\",\"numSequences\","
		 "\"blastVersion\",\"reference\",\"accession\",\"species\","
		 "\"specimen\",\"gene\",\"numMatches\",\"numGaps\","
		 "\"numBasePairs\",\"percentId\"\n"
		);
    break;
  }

  handleSamples(buffer,demuxFilePtr,resultDirPtr,resultsDirCPtr);
